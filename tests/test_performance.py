...
def test_caching():
    def time_test(label, format, ntests):
        ...
        for i in range(ntests):
            _ = str(Fmt(format))
        _t = time.time()-_t
        # print "%s caching=%s compile_caching=%s %d loops in %.3f seconds, %.03f msec/loop" % (label, Fmt.caching, Fmt.compile_caching, ntests, _t, _t*1000/ntests)
        # print Fmt.showstat(sys.stdout)
        return _t
    print
    for label, format in [("simple format", simple_format), ("complex format", complex_format)]:
        ...
        # for t,condition in times:
        #     print "%14s: parse cache:%-5s compile cache:%-5s" % condition, t
        if "simple" in label:
